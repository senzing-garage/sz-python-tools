...
class G2CmdShell(cmd.Cmd, object):
        ...
        if not defaultConfigID:
            ...
            try:
                ...
                self.g2_configmgr.addConfig(
                    config_string, "New default configuration added by G2ConfigTool.", addconfig_id
                )
        ...
        if not ftypeRecord:
            ...
            if felemRecord:
                fbomRecord = self.getRecord(
                    "CFG_FBOM", ["FTYPE_ID", "FELEM_ID"], [ftypeRecord["FTYPE_ID"], felemRecord["FELEM_ID"]]
                )
                if fbomRecord:
    ...
    def do_addDataSource(self, arg):
        ...
        parmData["RETENTIONLEVEL"], message = self.validateDomain(
            "Retention level", parmData.get("RETENTIONLEVEL", "Remember"), ["Remember", "Forget"]
        )
    ...
    def do_addFeature(self, arg):
        ...
        parmData["MATCHKEY"], message = self.validateDomain(
            "MatchKey", parmData.get("MATCHKEY", matchKeyDefault), ["Yes", "No", "Confirm", "Denial"]
        )
        ...
        for parmCode in parmData:
            ...
            if parmCode == "CANDIDATES":
                ...
                if not parmData["MATCHKEY"]:
                    ...
                    ftypeRecord, update_cnt = self.update_if_different(
                        ftypeRecord, update_cnt, "SHOW_IN_MATCH_KEY", parmData["MATCHKEY"]
                    )
                ...
                if not behaviorData:
                    ...
                    ftypeRecord, update_cnt = self.update_if_different(
                        ftypeRecord, update_cnt, "FTYPE_EXCL", behaviorData["EXCLUSIVITY"]
                    )
                    ftypeRecord, update_cnt = self.update_if_different(
    ...
    def do_addElement(self, arg):
        ...
        parmData["DATATYPE"], message = self.validateDomain(
            "DataType", parmData.get("DATATYPE", "string"), ["string", "number", "date", "datetime", "json"]
        )
    ...
    def do_deleteElement(self, arg):
        ...
        if fbomRecordList:
            featureList = ",".join(
                (self.getRecord("CFG_FTYPE", "FTYPE_ID", x["FTYPE_ID"])["FTYPE_CODE"] for x in fbomRecordList)
            )
    ...
    def do_addElementToFeature(self, arg):
        ...
        if parmData.get("EXECORDER") and execOrder != parmData["EXECORDER"]:
            colorize_msg(
                "The specified execution order is already taken  (remove it to assign the next available)", "error"
            )
            return
        else:
    ...
    def do_setFeatureElement(self, arg):
        ...
        oldParmData = {"DERIVED": oldRecord["DERIVED"], "DISPLAY": "No" if oldRecord["DISPLAY_LEVEL"] == 0 else "Yes"}
    ...
    def getCallID(self, feature, call_type, function=None):
        call_table, bom_table, call_id_field, func_table, func_code_field, func_id_field = self.setCallTypeTables(
            call_type
        )
    ...
    def prepCallElement(self, arg):
        ...
        parmData["CALLTYPE"], message = self.validateDomain(
            "Call type", parmData.get("CALLTYPE"), ["expression", "comparison", "distinct"]
        )
        ...
        bomRecord = self.getRecord(
            bom_table, [call_id_field, "FTYPE_ID", "FELEM_ID"], [parmData["ID"], ftypeID, felemID]
        )
        callElementData = {
    ...
    def do_addStandardizeCall(self, arg):
        ...
        sfcallOrder = self.getDesiredValueOrNext(
            "CFG_SFCALL", ["FTYPE_ID", "FELEM_ID", "EXEC_ORDER"], [ftypeID, felemID, parmData.get("EXECORDER")]
        )
    ...
    def formatExpressionCallJson(self, efcallRecord):
        ...
        for efbomRecord in sorted(
            self.getRecordList("CFG_EFBOM", "EFCALL_ID", efcallID), key=lambda k: k["EXEC_ORDER"]
        ):
    ...
    def do_addExpressionCall(self, arg):
        ...
        efcallOrder = self.getDesiredValueOrNext(
            "CFG_EFCALL", ["FTYPE_ID", "FELEM_ID", "EXEC_ORDER"], [ftypeID, felemID, parmData.get("EXECORDER")]
        )
        ...
        parmData["ISVIRTUAL"], message = self.validateDomain(
            "Is virtual", parmData.get("ISVIRTUAL", "No"), ["Yes", "No", "Any", "Desired"]
        )
            ...
            if elementData.get("ELEMENT") and elementData.get("ELEMENT").upper() != "N/A":
                if bom_ftypeID > 0:
                    bom_felemRecord, message = self.lookupFeatureElement(
                        elementData.get("FEATURE").upper(), elementData["ELEMENT"].upper()
                    )
                else:
    ...
    def do_listExpressionCalls(self, arg):
        ...
        for efcallRecord in sorted(
            self.getRecordList("CFG_EFCALL"), key=lambda k: (k["FTYPE_ID"], k["FELEM_ID"], k["EXEC_ORDER"])
        ):
    ...
    def formatComparisonCallJson(self, cfcallRecord):
        ...
        for cfbomRecord in sorted(
            self.getRecordList("CFG_CFBOM", "CFCALL_ID", cfcallID), key=lambda k: k["EXEC_ORDER"]
        ):
    ...
    def formatDistinctCallJson(self, dfcallRecord):
        ...
        for dfbomRecord in sorted(
            self.getRecordList("CFG_DFBOM", "DFCALL_ID", dfcallID), key=lambda k: k["EXEC_ORDER"]
        ):
    ...
    def do_listGenericThresholds(self, arg):
        ...
        for thresholdRecord in sorted(
            ...
            key=lambda k: (k["GPLAN_ID"], self.valid_behavior_codes.index(k["BEHAVIOR"])),
        ):
    ...
    def do_addGenericThreshold(self, arg):
        ...
        if self.getRecord(
            "CFG_GENERIC_THRESHOLD", ["GPLAN_ID", "BEHAVIOR", "FTYPE_ID"], [gplan_id, parmData["BEHAVIOR"], ftypeID]
        ):
    ...
    def validateRule(self, record):
        ...
        if record["RELATE"] == "Yes":
            ...
            if rtypeID not in (2, 3, 4):
                colorize_msg(
                    "Relationship type (RTYPE_ID) must be set to either 2=Possible match or 3=Possibly related", "error"
                )
                return None
        return record
                ...
                json_lines.append(
                    {
                        ...
                        "class": self.getRecord("CFG_RCLASS", "RCLASS_ID", rtypeRecord["RCLASS_ID"])["RCLASS_DESC"],
                    }
                )
    ...
    def do_addComparisonThreshold(self, arg):
        ...
        cfcallRecord = self.getRecord(
            "CFG_CFRTN", ["CFUNC_ID", "CFUNC_RTNVAL", "FTYPE_ID"], [cfuncID, parmData["SCORENAME"], ftypeID]
        )
        if cfcallRecord:
            colorize_msg(
        ...
        cfcallRecord = self.getRecord(
            "CFG_CFRTN", ["CFUNC_ID", "CFUNC_RTNVAL", "FTYPE_ID"], [cfuncID, parmData["SCORENAME"], 0]
        )
        if cfcallRecord:
    ...
    def do_deleteComparisonThreshold(self, arg):
        ...
        try:
            ...
            parmData["ID"] = (
                int(parmData["ID"]) if isinstance(parmData["ID"], str) and parmData["ID"].isdigit() else parmData["ID"]
            )
            self.validate_parms(parmData, ["ID"])
        except Exception as err:
            colorize_msg(f"Command error: {err}", "error")
    ...
    def print_json_lines(self, json_lines, display_header=""):
        ...
        if self.current_output_format_list == "table":
            ...
            for line in json_lines:
                if self.pygmentsInstalled:
                    render_string += (
                        highlight(json.dumps(line), lexers.JsonLexer(), formatters.TerminalFormatter()).replace(
                            "\n", ""
                        )
                        + "\n"
                    )
...
if __name__ == "__main__":
    ...
    argParser.add_argument(
        "-c", "--ini-file-name", dest="ini_file_name", default=None, help="name of a G2Module.ini file to use"
    )
    argParser.add_argument(
        "-f",
        "--force",
    ...
    argParser.add_argument(
        "-H", "--histDisable", dest="histDisable", action="store_true", default=False, help="disable history file usage"
    )
