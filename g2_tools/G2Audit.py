...
def makeKeytable(fileName, tableName):
    ...
    with open(fileMap["fileName"], "r") as csv_file:
        ...
        for row in csv_reader:
            ...
            if (
                fileMap["algorithmName"] == "Senzing"
                and "RELATED_ENTITY_ID" in rowData
                and rowData["RELATED_ENTITY_ID"] != "0"
            ):
...
def erCompare(fileName1, fileName2, outputRoot):
    ...
    for side2clusterID in fileMap2["clusters"]:
        ...
        if entityCnt % 10000 == 0:
            ...
            eps = int(
                float(sqlCommitSize) / (float(time.time() - batchStartTime if time.time() - batchStartTime != 0 else 1))
            )
            batchStartTime = time.time()
        ...
        if debugOn:
            ...
            print(
                ...
                % (side2clusterID, side2recordCnt, ",".join(sorted(side2recordIDs)[:10]))
            )
        ...
        if debugOn:
            print(
                ...
                % (side1recordCnt, side1clusterCnt, ",".join(map(str, side1clusterIDs.keys())))
            )
        ...
        if side2recordCnt > 1:
            ...
            if len(side1clusterIDs) > 1:  # --gonna be some new negatives here
                ...
                for clusterID in side1clusterIDs:
                    if (not largestSide1clusterID) or side1clusterIDs[clusterID] > side1clusterIDs[
                        largestSide1clusterID
                    ]:
                        largestSide1clusterID = clusterID
                statpack["PAIRS"]["COMMON_COUNT"] += (
                    side1clusterIDs[largestSide1clusterID] * (side1clusterIDs[largestSide1clusterID] - 1)
                ) / 2
