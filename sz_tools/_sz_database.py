...
class SzDatabase:
    ...
    def dburi_parse(self, node, dburi):
        ...
        try:
            ...
            (uri_dict["USERID"], uri_dict["PASSWORD"]) = (
                justUidPwd.split(":") if ":" in justUidPwd else (justUidPwd, "")
            )
            ...
            raise Exception(
                f"Failed to parse database URI, check the connection string(s) in your G2Module INI file."
            ) from None
